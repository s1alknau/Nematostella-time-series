...
MV_E_BUF_INVALID = (
    0x80000011  # < \~chinese 无效的缓存地址           \~english Buffer address invalid
)
MV_E_NOALIGN_BUF = (
    0x80000012  # < \~chinese 缓存对齐异常             \~english Buffer alignmenterror error
)
MV_E_NOENOUGH_BUF_NUM = (
    0x80000013  # < \~chinese 缓存个数不足             \~english Insufficient cache count
)
