...
MV_GENTL_CAMERALINK_DEVICE = (
    0x00000080  # < \~chinese CameraLink设备            \~english GenTL CameraLink Device
)
MV_GENTL_CXP_DEVICE = (
    0x00000100  # < \~chinese CoaXPress设备             \~english GenTL CoaXPress Device
)
MV_GENTL_XOF_DEVICE = 0x00000200  # < \~chinese XoF设备                   \~english GenTL XoF Device

# \~chinese 采集卡类型         \~english Interface type
MV_GIGE_INTERFACE = (
    0x00000001  # < \~chinese GigE Vision采集卡        \~english GigE Vision interface
)
MV_CAMERALINK_INTERFACE = (
    0x00000004  # < \~chinese Camera Link采集卡        \~english Camera Link interface
)
...
INFO_MAX_BUFFER_SIZE = (
    64  # < \~chinese 最大的数据信息大小           \~english Maximum data information size
)
...
MV_MAX_SERIAL_PORT_NUM = (
    64  # \~chinese 最大支持的串口数量  \~english The maximum number of serial port supported
)

MV_MAX_GENTL_IF_NUM = (
    256  # < \~chinese 最大支持的GenTL数量          \~english The maximum number of GenTL supported
)
...
MAX_EVENT_NAME_SIZE = (
    128  # < \~chinese 设备Event事件名称最大长度 \~english Max length of event name
)
MV_MAX_XML_SYMBOLIC_NUM = (
    64  # \~chinese 最大XML符号数               \~english Max XML Symbolic Number
)
MV_MAX_SYMBOLIC_LEN = (
    64  # \~chinese 最大枚举条目对应的符号长度     \~english Max Enum Entry Symbolic Number
)
